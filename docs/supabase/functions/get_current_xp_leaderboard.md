...
- **XP = tous les gains** (migration `058_leaderboard_include_all_gains`, 03/07/2026) : les MV sous-jacentes agrègent désormais `gains` par `customer_id` + `created_at` ∈ période (comme `user_stats`), et non plus via `receipts`. Les XP de quêtes, bonus et actions utilisateur (gains avec `receipt_id IS NULL`) comptent dans le classement — le total est cohérent avec celui affiché sur les profils.
