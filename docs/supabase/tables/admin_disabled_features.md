...
- **Onglet « Administrateurs » de `/settings`** (visible super admin uniquement, deep-link `?tab=admins`, entrée sidebar « Gestion des admins » dans le groupe Système) : matrice d'interrupteurs admins × fonctionnalités. Service : `src/lib/services/adminAccessService.ts`.
